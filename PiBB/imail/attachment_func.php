...
require_once("../conf/upload_conf.php");
...
function get_tmp_upload_dir($username) {
    global $_UPLOAD_ROOT;
    return "$_UPLOAD_ROOT/imail/tmp/$username";
}
...
function get_bbs_usr_upload_dir($username) {
    global $_UPLOAD_ROOT;
    return "$_UPLOAD_ROOT/imail/fin/$username";
}

//
// The final storage folder for BBS upload of a user's post in a forum.
//
function get_bbs_upload_dir($username, $mail_id, $salt) {
    $usr_dir = get_bbs_usr_upload_dir($username);
    return "$usr_dir/m$mail_id" . "_$salt";
}



//
// Show attachment files of a post.
// Note definition of tmp/bbs upload folders are in ../conf/upload_conf.php
//
function get_attachment($username, $mail_id, $salt) {
...
//
function delete_attachment($username, $mail_id, $salt) {
    $dir = get_bbs_upload_dir($username, $mail_id, $salt);
...
//
function insert_attachment($username, $mail_id, $salt) {
    if ($username == "" || $mail_id == "" || $salt == "") return 0;

    global $_UPLOAD_ROOT;
    $dir_src = get_tmp_upload_dir($username); 
    $dir_dst = get_bbs_upload_dir($username, $mail_id, $salt);

    //print "$dir_src, $dir_dst<br/>"; return;
