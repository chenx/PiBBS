...
require_once("../bbs/attachment_func.php");
...
if ($debug) {
    print "mode=$mode, fid=$fid, pid=$pid, user=$user, readonly=$readonly, debug=$debug<br/>";
}

$dir = get_upload_folder($mode, $fid, $pid, $user);
if ($dir == "") {
...
//
function get_upload_folder($mode, $forum_id, $post_id, $username) {
    global $error;
    if ($username == "") { 
        $error = "no username"; 
        return ""; 
    ...
    if ($mode == "1") {
        ...
        $salt = get_post_salt($forum_id, $post_id);
        if ($salt == "") { 
            $error = "no salt"; 
            return ""; 
        }
...
function get_post_salt($forum_id, $post_id) {
    $sql = "SELECT name FROM BBS_BoardList WHERE ID = " . db_encode($forum_id);
    $forum_name = executeScalar($sql);
    $sql = "SELECT salt FROM $forum_name WHERE ID = " . db_encode($post_id);
